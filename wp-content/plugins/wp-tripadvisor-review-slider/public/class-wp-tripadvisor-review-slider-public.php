 ...
 * @subpackage WP_TripAdvisor_Review/public
 */
 ...
 */
class WP_TripAdvisor_Review_Public {
	...
	public function enqueue_styles() {
		...
		foreach ( $templatearray as $template ){
			if(isset($template->style)){
				if($template->style=="1" || $template->style=="2" || $template->style=="3" || $template->style=="4" || $template->style=="5" || $template->style=="6" || $template->style=="7" || $template->style=="8" || $template->style=="9" || $template->style=="10"){
					wp_register_style( 'wp-tripadvisor-review-slider-public_template'.$template->style, plugin_dir_url( __FILE__ ) . 'css/wprev-public_template'.$template->style.'.css', array(), $this->version, 'all' );
					wp_enqueue_style( 'wp-tripadvisor-review-slider-public_template'.$template->style );
				}
			}
		}
		...
		wp_register_style( 'wptripadvisor_w3', plugin_dir_url( __FILE__ ) . 'css/wptripadvisor_w3.css', array(), $this->version, 'all' );

		wp_enqueue_style( 'wptripadvisor_w3' );
		wp_enqueue_style( 'unslider' );
		wp_enqueue_style( 'unslider-dots' );
		*/
		//combine all css in to one.
		wp_register_style( 'wptripadvisor_allcss', plugin_dir_url( __FILE__ ) . 'css/wptripadvisor_all.css', array(), $this->version, 'all' );
		wp_enqueue_style( 'wptripadvisor_allcss' );
	...
	public function enqueue_scripts() {
		 ...
		 * The WP_TripAdvisor_Review_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class. wprevpublicjs_script_vars
		 */
	 ...
	 * @since    1.0.0
	 */
	public function shortcode_wptripadvisor_usetemplate() {
	
				add_shortcode( 'wptripadvisor_usetemplate', array($this,'wptripadvisor_usetemplate_func') );
	}	 
	public function wptripadvisor_usetemplate_func( $atts, $content = null ) {
		//get attributes
