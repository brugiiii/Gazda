...
class wptripadvisor_Widget extends WP_Widget {

  // Set up the widget name and description.
  public function __construct() {
    $widget_options = array( 'classname' => 'wptripadvisor_widget', 'description' => 'Widget for WP TripAdvisor Review Slider Plugin. Allows you to display your reviews in your widget areas.' );
    parent::__construct( 'wptripadvisor_widget', 'WP TripAdvisor Review Slider', $widget_options );
  }
  ...
  public function widget( $args, $instance ) {
	...
	if($templatedetails){
		...
		include plugin_dir_path( __FILE__ ) . 'partials/wp-tripadvisor-review-slider-public-display-widget.php';
...
// Register the widget.
function wptripadvisor_register_widget() { 
  register_widget( 'wptripadvisor_Widget' );
}
global $wpdb;
//register a widget if there is a review template or badge that is type widget.
$table_name = $wpdb->prefix . 'wptripadvisor_post_templates';
$templatewidgetrowcounttr = $wpdb->get_var("SELECT COUNT(*) FROM $table_name WHERE template_type='widget'");
if($templatewidgetrowcounttr>0){
	add_action( 'widgets_init', 'wptripadvisor_register_widget' );
}
?>